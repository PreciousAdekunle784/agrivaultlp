...
function handleGoogleSignIn() {
    ...
    if (typeof google === 'undefined' || !google.accounts) {
        // GIS not loaded — tell user to set up Client ID
        showAuthError('Google Sign-In is not configured yet. Please set your Google Cloud Client ID in app.js (CONFIG.GOOGLE_CLIENT_ID).');
        return;
    }
    ...
    try {
        google.accounts.id.initialize({
            ...
            callback: async (response) => {
                try {
                    const user = await AuthManager.googleSignIn(response);
                    showApp(user);
                } catch (err) {
        ...
        google.accounts.id.prompt((notification) => {
            if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                // One Tap not available, try rendering a button
                showAuthError('Google Sign-In popup was blocked or dismissed. Please try again or use email sign-in.');
            }
        });
    } catch (err) {
        showAuthError('Google Sign-In failed. Please check your Client ID configuration.');
    }
}
